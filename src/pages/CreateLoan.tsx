...
export default function CreateLoan() {
  ...
  const handleAddMarket = () => {
    ...
    setEmbeddedMarkets([...embeddedMarkets, {
      url: newMarketUrl,
      slug
    }]);
    setNewMarketUrl("");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Get user's Manifold settings for username (API key no longer required for loan creation)
      const {
        data: settings
      } = await supabase.from("user_manifold_settings").select("manifold_username").eq("user_id", user.id).maybeSingle();
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <form onSubmit={handleSubmit} className="lg:col-span-2 space-y-6">
            ...
            animationDelay: "100ms"
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input id="title" placeholder="e.g., Bail out Tumbles" value={formData.title} onChange={e => {
                  setFormData({
                    ...formData,
                ...
                }} className={cn("bg-secondary/50", validationErrors.title && "border-destructive")} maxLength={100} />
                ...
                <div className="space-y-2">
                  ...
                  <Textarea id="description" placeholder="Explain your trading strategy and why investors should fund your loan..." value={formData.description} onChange={e => {
                  setFormData({
                    ...formData,
                ...
                }} className={cn("bg-secondary/50 min-h-[120px]", validationErrors.description && "border-destructive")} maxLength={2000} />
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <Input placeholder="https://manifold.markets/user/market-slug" value={newMarketUrl} onChange={e => setNewMarketUrl(e.target.value)} className="bg-secondary/50" onKeyPress={e => e.key === "Enter" && (e.preventDefault(), handleAddMarket())} />
                    <Button type="button" variant="outline" onClick={handleAddMarket}>
                      <Plus className="w-4 h-4" />
                    </Button>
                  </div>
                  {embeddedMarkets.length > 0 && <div className="space-y-2">
                      {embeddedMarkets.map(market => <div key={market.slug} className="flex items-center justify-between p-3 rounded-lg bg-secondary/30 border border-border/50">
              ...
              <CardContent className="space-y-6">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Input value={`M$${formData.amount.toLocaleString()}`} onChange={e => handleAmountInput(e.target.value)} className={cn("w-32 text-right font-bold bg-secondary/50 h-8", validationErrors.amount && "border-destructive")} />
                  </div>
                  {validationErrors.amount && <p className="text-xs text-destructive">{validationErrors.amount}</p>}
                  <Slider value={[Math.min(formData.amount, 100000)]} onValueChange={value => {
                  setFormData({
                    ...formData,
                    amount: value[0]
                  });
                ...
                }} min={100} max={100000} step={100} className="py-4" />
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Input value={`${formData.interestRate}%`} onChange={e => handleInterestInput(e.target.value)} className={cn("w-24 text-right font-bold text-success bg-secondary/50 h-8", validationErrors.interestRate && "border-destructive")} />
                  </div>
                  {validationErrors.interestRate && <p className="text-xs text-destructive">{validationErrors.interestRate}</p>}
                  <Slider value={[Math.min(Math.max(formData.interestRate, 1), 25)]} onValueChange={value => {
                  setFormData({
                    ...formData,
                ...
                }} min={1} max={25} step={1} className="py-4" />
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span>1%</span>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Input value={`${formData.termDays} days`} onChange={e => handleTermInput(e.target.value)} className={cn("w-28 text-right font-bold bg-secondary/50 h-8", validationErrors.termDays && "border-destructive")} />
                      <Popover>
                        ...
                        <PopoverContent className="w-auto p-0" align="end">
                          <Calendar mode="single" selected={maturityDate} onSelect={handleDateSelect} disabled={date => date < new Date()} initialFocus className={cn("p-3 pointer-events-auto")} />
                        </PopoverContent>
                      </Popover>
                    </div>
                  </div>
                  {validationErrors.termDays && <p className="text-xs text-destructive">{validationErrors.termDays}</p>}
                  <Slider value={[Math.min(Math.max(formData.termDays, 7), 180)]} onValueChange={value => {
                  setFormData({
                    ...formData,
                    termDays: value[0]
                  });
                ...
                }} min={7} max={180} step={1} className="py-4" />
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span>7 days</span>
                    <span>180 days (max 365)</span>
                  </div>
                  {maturityDate && <p className="text-sm text-muted-foreground">
                ...
                <div className="space-y-4 pt-4 border-t border-border/50">
                  <div className="flex items-center justify-between">
                    ...
                    <Input value={`${formData.fundingPeriodDays} days`} onChange={e => {
                    const num = parseInt(e.target.value.replace(/[^0-9]/g, ""), 10);
                    if (!isNaN(num) && num >= 1) {
                      setFormData({
                        ...formData,
                        fundingPeriodDays: Math.min(num, 30)
                      });
                  ...
                  <Slider value={[formData.fundingPeriodDays]} onValueChange={value => {
                  setFormData({
                    ...formData,
                    fundingPeriodDays: value[0]
                  });
                  setFundingDeadline(addDays(new Date(), value[0]));
                }} min={1} max={30} step={1} className="py-4" />
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span>1 day</span>
                    <span>30 days</span>
                  </div>
                  {fundingDeadline && <p className="text-sm text-muted-foreground">
                      Funding deadline:{" "}
            ...
            animationDelay: "300ms"
              ...
              <CardContent>
                <Textarea placeholder="e.g., I have to take one of your dares at Manifest 2026" value={formData.collateralDescription} onChange={e => setFormData({
                ...formData,
                collateralDescription: e.target.value
              })} className="bg-secondary/50" maxLength={1000} />
