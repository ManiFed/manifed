...
const Index = () => {
  ...
  const fetchLoans = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('loans').select('*').order('created_at', {
        ascending: false
      });
  ...
  return <div className="min-h-screen">
      ...
      <main className="container mx-auto px-4 py-8 space-y-8">
        ...
        {/* Filters */}
        <LoanFilters activeStatus={activeStatus} onStatusChange={setActiveStatus} searchQuery={searchQuery} onSearchChange={setSearchQuery} />
        ...
        <section className="space-y-4">
          ...
          {isLoading ? <div className="flex items-center justify-center py-16">
            ...
            </div> : transformedLoans.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {transformedLoans.map((loan, index) => <div key={loan.id} className="animate-slide-up" style={{
            animationDelay: `${index * 100}ms`
          }}>
                  <LoanCard loan={loan} />
                </div>)}
            </div> : <div className="text-center py-16 glass rounded-xl">
              <TrendingUp className="w-12 h-12 text-muted-foreground mx-auto mb-4" />
              <h3 className="text-lg font-medium text-foreground mb-2">No loans found</h3>
              <p className="text-muted-foreground">
                {loans.length === 0 ? 'Be the first to create a loan!' : 'Try adjusting your filters or search query'}
              </p>
            </div>}
        </section>
      </main>
