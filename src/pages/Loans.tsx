...
export default function Loans() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch bond issuers
      const { data: issuersData } = await supabase.from("bond_issuers").select("*").order("name");
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "bonds" | "p2p")} className="w-full">
          ...
          <TabsContent value="bonds" className="space-y-6">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <Select
                value={selectedIssuer === "all" ? issuers[0]?.id || "all" : selectedIssuer}
                      ...
                      {BOND_TERMS.map((term) => {
                        ...
                        return (
                          <button
                            ...
                            className={`p-4 rounded-lg border-2 text-left transition-all ${isSelected ? "border-primary bg-primary/10" : "border-border/50 bg-secondary/30 hover:border-primary/50"}`}
                                ...
                                <div key={bond.id} className="p-3 rounded-lg bg-secondary/30">
                                  ...
                                  {bond.status === "active" && bond.next_interest_date && (
                                    <p className="text-xs text-success mt-1">
                                      Next: M${bondMonthlyInterest.toFixed(2)} on{" "}
                                      {format(new Date(bond.next_interest_date), "MMM d")}
                                    </p>
                                  )}
                                </div>
                              );
          ...
          <TabsContent value="p2p" className="space-y-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <Select value={loanFilter} onValueChange={setLoanFilter}>
                  ...
                  <SelectContent>
                    <SelectItem value="seeking_funding">Seeking/Offering Funding</SelectItem>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {transformedLoans.length === 0 ? (
                ...
                transformedLoans.map((loan) => <LoanCard key={loan.id} loan={loan} />)
              )}
            </div>
          </TabsContent>
        </Tabs>
