    ...
    try {
      const { data, error } = await supabase.functions.invoke("managram", {
        body: {
          ...
          message: `${depositAmount} mana deposit to ManiFed account`,
        },
      });
      ...
      <PopoverContent className="w-72 p-4" align="end">
        {!hasApiKey ? (
          <div className="text-center space-y-3">
            <p className="text-sm text-muted-foreground">Connect your Manifold account to deposit and withdraw M$.</p>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="font-medium text-foreground">{mode === "deposit" ? "Deposit M$" : "Withdraw M$"}</h4>
              <Button
                variant="ghost"
                size="sm"
                onClick={() => {
